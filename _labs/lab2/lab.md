...
(c) pointer/bitwise expressions to set GPIO registers, and

(d) if/else versus switch.

You could review the assembly in the `.list` files generated by
running `make`, but it's a bit faster (and definitely more fun!) to
play with the online Compiler Explorer we saw in class.
...
from `codegen.c` in, one by one, and see if you can figure out what
the generated assembly is doing.

During lab time, we'd like you and your partner to work through the
first four parts (a)-(c), and (d) if possible.  Use the comments in
...
Look at `testing.c`, `assert.h`, and `abort.c`. In particular, notice
the simple unit tests in `main()` in `testing.c`. The things we pass
to `assert()` should always be true if our program is written
properly.
