...
3. Read section 3.1 in this lab on the theory of operation
   for 7-segment displays. Also, skim the rest of section 3.
...
Go to the `codegen` directory. Review the C code in the `codegen.c` source file.
...
Running `make` will generate both `codegen.bin` and
 `codegen.list`. To produce `codegen.bin`, we need to
...
`codegen.list` you can look at. A `.list` file contains a listed representation
of the ARM assembly instructions compiled from the corresponding C source file.
...
The comments in `codegen.c` guide you through four issues: (a) if/else versus switch,
(b) simple loops, (c) comparing pointer/array accesses, and
(d) pointer/bitwise expressions to set GPIO registers.

During lab time, we'd like you and your partner to work through the first four parts if possible. 
Use the comments in the C source as your guide. If time is running short, it's ok to leave a remaining part or two for later completion on your own.

You can open the list files generated by our Makefile to review the assembly after your compile, but it's a bit faster (and definitely more fun!) to play with the online [Compiler Explorer](https://gcc.godbolt.org/). Choose the compiler for `ARM gcc 5.4` and enter flags `-Og -ffreestanding -marm`. This gives you a pretty close approximation of our compiler version/environment (although not an exact match).
...
#### 3. Setup up a 4-digit 7-segment display (75 min)
...
For parts 3.2, and 3.3, feel free to use jumpers for ease of debugging!
...
#### 3.2. Wire up the resistors to the segment pins of the display
...
#### 3.3. Wire up the transistors to the digit pins
...
#### 3.4. Permanently wire up the circuit
...
#### 3.5. Connect the display to the Raspberry Pi
...
2. Go through the codegen Makefile.

3. Walk through [this secret part (e)](https://godbolt.org/g/35xBV9).