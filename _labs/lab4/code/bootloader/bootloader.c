...
static unsigned char getbyte(void) { 
	...
	// while uart not ready, just keep going; nak every sec
	while(((uart_lcr() & 0x01) == 0)) {
        if ((timer_tick() - t0) >= 1000000) {
            act_led_heartbeat();    // two pulses of act led to signal alive
            uart_send(NAK);
            t0 = timer_tick();
    	}
	}
    act_led_on();   // turn act led on solid when going into receive
    return uart_recv();
}
...
void notmain ( void ) {
	...
	while (1) {
    	...
    	if((b = getbyte()) == EOT) {
            act_led_off();  // turn off act led when receive complete
      		uart_send(ACK);
      		// if uploaded program immediately starts communicating 
      		// over uart, can confuse bootloader client.
      		// pause here for bootloader client to receive/process ACK
      		// only then do we start running the uploaded program
      		unsigned t0 = timer_tick();
      		while ((timer_tick() - t0) < 50000) ; // half-sec
      		BRANCHTO(ARMBASE);
			return; // NOTREACHED
		}
