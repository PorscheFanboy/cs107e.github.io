...
static struct node *open(char *path) {
    ...
    if (path[0] == '/' && path[1] == '\0') {
        ...
        printf("ERROR: Non-absolute path passed to open().\n");
        return NULL;
    }
...
char buf[5000];
void fs_test_dir(char *path) {
    dir_t *dir;
    dir_read_entry_t entry;

    dir = fs_dir_open(path);
    printf("\n%s:\n", path);
...
void fs_test_file(char *path) {
    file_t *fd = fs_file_open(path);
    if (fd == NULL) {
        printf("File %s not found.\n", path);
        return;
    }

    printf("\n%s (cluster %d) (size %d):\n", path, fd->node.start_cluster, fd->node.size);
    unsigned read_bytes = fs_file_read(fd, buf, 5000);
    printf("Read %d bytes:\n", read_bytes);
    for (int i = 0; buf[i] != 0 && i < read_bytes; i++) {
        printf("%c", buf[i]);
    }
    printf("\n");
...
int fs_test(int console) {
    fs_test_dir("/");
    fs_test_dir("/libpi/src/");

    fs_test_file("/policies.md");
    fs_test_file("/home.MD");
    fs_test_file("/libpi/src/abort.c");
    fs_test_file("/blahblah");
